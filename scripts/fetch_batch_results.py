"""Download completed Together AI batch jobs and print average LLM-judge scores.

Usage:
    python scripts/fetch_batch_results.py <batch_job_id_1> [batch_job_id_2 ...] [--save PATH] [--report PATH]

The script fetches the output files, combines them, parses each response, and reports per-dataset
and overall averages for the four evaluation dimensions.  For any response where
the JSON scores cannot be parsed, the raw content is printed and you are prompted
    ...
    for line in content.splitlines():
        ...
        try:
            ...
            if content_str.startswith("```"):
                # Split at the first newline to remove the ```json line
...
def main():
    ...
    # Changed from a single positional argument to accepting a list using nargs='+'
    ...
    # Iterate through all provided batch IDs
    ...
    # Combine all JSONL strings into one massive blob
    ...
    # Pass the combined text blob into the parser
...
# """Download a completed Together AI batch job and print average LLM-judge scores.

# Usage:
#     python scripts/fetch_batch_results.py <batch_job_id> [--save PATH] [--report PATH]

# The script fetches the output file, parses each response, and reports per-dataset
# and overall averages for the four evaluation dimensions.  For any response where
# the JSON scores cannot be parsed, the raw content is printed and you are prompted
# to enter scores manually.
# """
...
#     if batch.progress is not None:
#         print(f"Progress: {batch.progress:.1f}%")
...
#     if not batch.output_file_id:
#         print("No output_file_id on completed batch.")
#         sys.exit(1)
...
# def main():
#     parser = argparse.ArgumentParser(description="Fetch and parse a Together AI batch evaluation job.")
#     parser.add_argument("batch_id", help="Together AI batch job ID")
#     parser.add_argument("--save", "-s", metavar="PATH", help="Save the raw output JSONL to this path")
#     parser.add_argument("--report", "-r", metavar="PATH", help="Save a formatted .txt evaluation report to this path")
#     args = parser.parse_args()

#     client = Together(api_key=os.environ.get("TOGETHER_API_KEY"))

#     raw_content = fetch_output(client, args.batch_id)
...
#     scores_by_dataset, n_ok, n_errors, n_parse_failures = parse_scores(raw_content)
